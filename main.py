...
import json
from core_func import get_transcription_from_url, get_transcription_from_audio, take_notes_chatgpt, create_notes_notion
...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.setting_icon_path = resource_path("./assets/gear.png")  # Update this path to your settings icon image file
    ...
    def main_interface(self):
        ...
        self.entry1 = ctk.CTkEntry(self.frame1, placeholder_text="URL/Video/Audio/Subtitle/Text File", width=250)
        ...
        self.opt_lan = ctk.CTkOptionMenu(self.frame4, values=["English", "简体中文", "繁體中文", "Español", "Français", "Deutsch", "Português", "Русский", "日本語", "العربية", "हिन्दी", "한국어", "Italiano"], width=150)
        ...
        self.opt_gpt = ctk.CTkOptionMenu(self.frame3, values=["GPT-3.5-turbo", "GPT-4o"], width=150)
        ...
        self.take_notes_button = ctk.CTkButton(self.button_frame, text="Take Notes", width=300, height=30, command=self.take_notes)
        self.take_notes_button.grid(row=0, column=0, padx=10)

        self.setting_button = ctk.CTkButton(self.button_frame, image=self.setting_icon, text="", width=30, height=30, command=self.switch_to_setting)
        self.setting_button.grid(row=0, column=1, padx=10)
        
        # Create and hide the console output
        self.console_output = ctk.CTkLabel(self.main_frame, text="", anchor="w", width=400, height=30)
    ...
    def take_notes(self):
        ...
        try:
            ...
            if user_input.startswith('https'):
                self.update_console_output("Getting the transcription....                    ")
                ...
                if user_input.endswith('.txt'):
                    ...
                    self.update_console_output("Getting the transcription....                    ")
                    logger.info("Getting the transcription...")
                    transcription = get_transcription_from_audio(user_input, self.opt_lan.get(), self.opt_whisper.get(), self.setting.get("chatgpt_api", ""), self.update_progress_bar, logger)
            
            self.update_progress_bar(0.9)   
            self.update_console_output("Taking the notes by ChatGPT....                    ")
            logger.info("Taking the notes by ChatGPT...")
            notes = take_notes_chatgpt(transcription, self.opt_lan.get(), self.setting.get("chatgpt_api", ""), logger=logger)
            self.update_progress_bar(1)
            self.update_console_output("Creating Notion page....                    ")
            logger.info("Creating Notion page...")

            create_notes_notion(notes, title, user_input, self.setting.get("notion_api", ""), self.setting.get("database_id", ""), logger=logger)
            self.update_console_output("Note-taking process completed!                    ")
    ...
    def setting_interface(self):
        ...
        self.label3.grid(row=0, column=0, padx=10)
        self.entry3 = ctk.CTkEntry(self.frame3, placeholder_text="Database ID", width=200)
        self.entry3.insert(0, setting.get("database_id", ""))
        self.entry3.grid(row=0, column=1, padx=10)

        self.save_button = ctk.CTkButton(self.main_frame, text="Save", command=self.save_api_setting)
