...
language_dict = {
    ...
    "繁體中文": ["zh", "zh-Hans", "zh-CN", "zh-SG", "zh-Hant", "zh-HK", "zh-MO", "zh-TW"],
    "English": [
        "en", "en-AU", "en-BZ", "en-CA", "en-029", "en-HK", "en-IN", "en-IE", "en-JM",
        "en-MY", "en-NZ", "en-PH", "en-SG", "en-ZA", "en-TT", "en-AE", "en-GB", "en-US", "en-ZW"
    ],
    "Español": [
        "es", "es-AR", "es-VE", "es-BO", "es-CL", "es-CO", "es-CR", "es-CU", "es-DO",
        "es-EC", "es-SV", "es-GT", "es-HN", "es-419", "es-MX", "es-NI", "es-PA", "es-PY",
        "es-PE", "es-PR", "es-ES", "es-US", "es-UY"
    ],
    "Français": [
        "fr", "fr-BE", "fr-CI", "fr-CM", "fr-CA", "fr-029", "fr-CD", "fr-FR", "fr-HT",
        "fr-LU", "fr-ML", "fr-MA", "fr-MC", "fr-RE", "fr-SN", "fr-CH"
    ],
    ...
    "日本語": ["ja", "ja-JP"],
    "العربية": [
        "ar", "ar-DZ", "ar-BH", "ar-EG", "ar-IQ", "ar-JO", "ar-KW", "ar-LB", "ar-LY",
...
def download_subtitle(youtube_url, language=['en'], output_path='./', logger=None):
    ...
    with youtube_dl.YoutubeDL(ydl_opts) as ydl:
        ...
        if 'subtitles' in info and find_matching_item(language, info['subtitles']):
            ...
            subtitle_file = ydl.prepare_filename(info).replace('.mp4', f'.{sub_language}.{subtitle_format}')
        else:
...
def convert_srt_vtt_to_text(srt_vtt_file):
    ...
    with open(srt_vtt_file, 'r', encoding='utf-8') as file:
        for line in file:
            if line.strip() and not line.startswith("WEBVTT") and 'Kind: captions' not in line and 'Language:' not in line:
...
def fasterWhisperTranscribe(file_path, language, model_size="medium.en", update_progress_bar=None, logger=None):
    if logger is None:
        logger = logging.getLogger(__name__)
    ...
    for line in lines:
        ...
        if line_strip.startswith('- '):
            blocks.append({
                ...
                "type": "heading_3",
                "heading_3": {
                    "rich_text": [{"type": "text", "text": {"content": line_strip[2:]}}]
                }
            })
...
def add_property_to_database(database_id, property_name, property_type, headers, logger=None):
    if logger is None:
        logger = logging.getLogger(__name__)
    url = f"https://api.notion.com/v1/databases/{database_id}"
    update_payload = {
    ...
    if response.status_code == 200:
        ...
        logger.warning(f"Failed to add property to database: {response.status_code}, {response.text}")
