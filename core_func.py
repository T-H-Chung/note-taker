...
import json
import logging
from utils import get_video_title, download_subtitle, get_file_format, convert_ass_to_text, convert_srt_vtt_to_text, download_audio, fasterWhisperTranscribe, whisperAPITranscribe, run_command, parse_input, split_text_by_token_limit_tiktoken, check_property_exists, add_property_to_database, language_dict

def get_transcription_from_audio(audio_file, language, model_size="medium", api_token="", update_progress_bar=None, logger=None):
    if logger is None:
        logger = logging.getLogger(__name__)
    if model_size == "Whisper API":
        transcription = whisperAPITranscribe(audio_file, language_dict[language][0], api_token, logger=logger)
        os.remove(audio_file)
    else:
        transcription = fasterWhisperTranscribe(audio_file, language_dict[language][0], model_size=model_size, update_progress_bar=update_progress_bar, logger=logger)
        os.remove(audio_file + ".txt")
    return transcription

def get_transcription_from_url(youtube_url, language, model_size="medium", api_token="", update_progress_bar=None, logger=None):
    if logger is None:
        logger = logging.getLogger(__name__)
    
    title = get_video_title(youtube_url)
    subtitle_file = download_subtitle(youtube_url, language=language_dict[language], logger=logger)
    ...
    if subtitle_file and os.path.exists(subtitle_file):
        ...
        transcription = get_transcription_from_audio(audio_file, language, model_size, api_token, update_progress_bar, logger)
        
    logger.info("Transcription process completed.")
    return transcription, title

def take_notes_chatgpt(transcription, language, api_token, model_name="GPT-3.5-turbo", save_reply=False, logger=None):
    if logger is None:
        logger = logging.getLogger(__name__)
    if save_reply:
    ...
    for chunk in user_chunks:
        ...
        command = ['curl', 'https://api.openai.com/v1/chat/completions', '-H', f'Authorization: Bearer {api_token}', '-H', "Content-Type: application/json", '-d', '@prompt.json']
        chatgpt_reply = run_command(command, logger=logger)
        messages.append(chatgpt_reply['choices'][0]['message'])
        messages.append({"role": "system", "content": "Following is the next part. Continue the task."})
...
def create_notes_notion(notes, title, youtube_url, notion_api_token, database_id, logger=None):
    if logger is None:
        logger = logging.getLogger(__name__)
    ...
    def append_apge(page_id, append_data):
        ...
        response = requests.patch(append_block_url, headers=headers, data=json.dumps(append_data))
        if response.status_code == 200:
            if len(append_data) > 99:
                append_apge(page_id, append_data[99:])
        else:
            logger.error("Failed to append page. Status code: %d. Response: %s", response.status_code, response.text)
    ...
    if response.status_code == 200:
        ...
        logger.error("Failed to create page. Status code: %d. Response: %s", response.status_code, response.text)
